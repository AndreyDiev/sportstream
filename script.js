...
ether.addEventListener('click', () => {
  translations.classList.toggle('_active')
})
...
const passwordViewBtnList = Array.from(document.querySelectorAll('.password-view'));
const passwordInputsList = Array.from(document.querySelectorAll('.popup-reg__input_type_password'));
console.log(passwordViewBtnList);

function handlePasswordView(input) {
...
passwordViewBtnList.forEach((viewElement) => {
  viewElement.addEventListener('click', (e) => {
    e.target.classList.toggle('password-view-unhidden');
    if (e.target.id === 'view1') {
      handlePasswordView(passwordInputsList[0]);
    }
    if (e.target.id === 'view2') {
      handlePasswordView(passwordInputsList[1]);
    }
    if (e.target.id === 'view3') {
      handlePasswordView(passwordInputsList[2]);
    }
  })
})
